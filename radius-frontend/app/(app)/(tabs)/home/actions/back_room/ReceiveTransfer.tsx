...
export default function ReceiveTransfer() {
    ...
    const fetchTransfer = useCallback(async () => {
        setIsLoading(true);
        const data = await callApi<StockTransferDetailResponse>(ENDPOINTS.SALES_FLOOR.RECEIVING.transfer(transfer_id as string), { method: "GET" }, logout);
        if (data) {
            setTransfer(data);
            setScannedItems({});
        }
    ...
    const handleScan = async (barcode: string) => {
        ...
        const res = await callApi<CheckProductInTransferResponse>(`${ENDPOINTS.SALES_FLOOR.RECEIVING.checkTransferProduct(transfer.transfer_id)}?barcode=${barcode}`, { method: "GET" }, logout);
        if (res) {
    ...
    const handleReceiveBatch = async () => {
        ...
        const res = await callApi<{ message: string }>(ENDPOINTS.SALES_FLOOR.RECEIVING.receiveTransfer(transfer.transfer_id), {
            method: "POST",
